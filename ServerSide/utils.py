...
def get_sine_wave(frequency, duration, sample_rate=44100, amplitude=4096):
    ...
    duration : float  משך
        Time in seconds.
    sample_rate : int, optional   קצב זרימה
...
def get_song_data(music_notes,note_values, bar_value, factor, length, decay, sustain_level, sample_rate=44100, amplitude=4096):
    ...
    bar_value: Duration of a bar.
    factor : Factor to be used to generate overtones.
    length : Stage length to be used to calculate ADSR weights.
    decay : Stage decay to be used to calculate ADSR weights.
    sustain_level : Amplitude of `S` stage as a fraction of max amplitude.
    sample_rate : Wav file sample rate. The default is 44100.
    amplitude : Peak Amplitude. The default is 4096.
    Returns
    song : ndarray
    '''
